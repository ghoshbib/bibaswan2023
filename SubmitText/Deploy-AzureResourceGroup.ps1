...
param(
 ...
 [string]
 $deploymentName = 'Submit Text Logic App Deployment',
...
#if(!$connectionExists)
#{
	New-AzResource -Properties @{"api" = @{"id" = "subscriptions/" + $subscriptionId + "/providers/Microsoft.Web/locations/" + $resourceGroupLocation + "/managedApis/" + 'azureeventgrid'}; "displayName" = 'SubmitText'; } -ResourceName 'SubmitText' -ResourceType "Microsoft.Web/connections" -ResourceGroupName $resourceGroupName -Location $resourceGroupLocation -Force

	$connection = Get-AzResource -ApiVersion '2016-06-01' -ResourceType "Microsoft.Web/connections" -ResourceGroupName $resourceGroupName -ResourceName "SubmitText"
...
Write-Host "Starting deployment of Submit Text Logic App";
...
Write-Host "Completed deployment of Submit Text Logic App";