...
param(
 ...
 [string]
 $deploymentName = 'Submit Question Logic App Deployment',
...
#$connectionExists = Get-AzResource -ApiVersion '2016-06-01' -ResourceType "Microsoft.Web/connections" -ResourceGroupName $resourceGroupName -ResourceName "SubmitQuestions" -ErrorAction SilentlyContinue

#if(!$connectionExists)
#{
	New-AzResource -Properties @{"api" = @{"id" = "subscriptions/" + $subscriptionId + "/providers/Microsoft.Web/locations/" + $resourceGroupLocation + "/managedApis/" + 'azureeventgrid'}; "displayName" = 'SubmitQuestions'; } -ResourceName 'SubmitQuestions' -ResourceType "Microsoft.Web/connections" -ResourceGroupName $resourceGroupName -Location $resourceGroupLocation -Force

	$connection = Get-AzResource -ApiVersion '2016-06-01' -ResourceType "Microsoft.Web/connections" -ResourceGroupName $resourceGroupName -ResourceName "SubmitQuestions"
...
Write-Host "Starting deployment of Submit Question Logic App";
...
Write-Host "Completed deployment of Submit Question Logic App";